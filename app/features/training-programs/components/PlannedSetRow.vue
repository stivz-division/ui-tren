...
<template>
  <div class="grid grid-cols-[44px_minmax(0,1fr)_minmax(0,1fr)_44px] items-start gap-2 tabular-nums">
    ...
    <label><span class="mb-1 block text-sm text-muted">Вес, кг</span><input v-model="model.workingWeightKg" inputmode="decimal" placeholder="0" class="min-h-12 w-full rounded-xl border border-default bg-default px-3 text-base" :aria-invalid="Boolean(errors?.[`${path}.working_weight_kg`])" :aria-describedby="errors?.[`${path}.working_weight_kg`] ? `${errorIdBase}-weight-error` : undefined" ><span v-if="errors?.[`${path}.working_weight_kg`]" :id="`${errorIdBase}-weight-error`" class="mt-1 block text-sm text-error" role="alert">{{ errors[`${path}.working_weight_kg`] }}</span></label>
