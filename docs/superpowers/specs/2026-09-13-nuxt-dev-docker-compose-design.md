...
Рабочая директория — `/app`. Dockerfile сначала копирует `package.json` и `pnpm-lock.yaml`, устанавливает зависимости через `pnpm install --frozen-lockfile`, затем копирует проект. Команда запуска — `pnpm dev --host 0.0.0.0`, внутренний порт — `3000`.
...
2. `docker compose build` — только если в проекте уже присутствуют `package.json` и `pnpm-lock.yaml`.
3. `docker compose up` и проверка `http://localhost:${APP_PORT:-3000}` — после появления минимального Nuxt-приложения.

Сейчас репозиторий ещё не содержит Nuxt scaffold и lockfile. Поэтому Docker-конфигурацию можно проверить статически, но полноценная сборка станет возможна после создания приложения.
