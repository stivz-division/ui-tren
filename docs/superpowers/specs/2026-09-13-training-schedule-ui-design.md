...
- В backend отсутствует публичный каталог упражнений. Production UI не содержит статических названий или идентификаторов. Клиент каталога изолируется за интерфейсом `ExerciseCatalogGateway` и обращается к будущему `GET /api/exercises` с `{ data: Array<{ id: number; name: string }> }`.
- Пока endpoint отсутствует, создание и добавление новых упражнений показывают явное состояние недоступности каталога с действием повторной загрузки. Уже сохранённые `exercise_id` отображаются безопасной подписью `Упражнение №<id>`, если имя не удалось разрешить.
