...
Из истории доступен анализ: план и факт, заключение, контекст прошлых тренировок и рекомендации. На подготовке рекомендации последней сессии выбранной программы можно применить или отклонить до старта. Применение меняет будущий план, сохраняя историю и снимок активной тренировки. Незавершённые этапы автоматически перечитываются только на открытом видимом экране; «Обновить» не запускает повторную генерацию. Ограничения API описаны в [контракте](.ai-factory/references/api-tren-contract.md#анализ-и-рекомендации-2026-09-22).
